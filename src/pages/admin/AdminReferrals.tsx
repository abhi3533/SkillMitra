...
import { Users, Gift, IndianRupee, TrendingUp, Search, AlertTriangle } from "lucide-react";
...
const AdminReferrals = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const totalCredits = sData.filter(r => r.status === "paid").length * 200 + tData.filter(r => r.status === "paid").length * 500;
      ...
      enrichedS.forEach(r => {
        ...
        if (r.status === "paid") referrerCounts[r.referrer_id].earned += 200;
      });
    ...
    <AdminLayout>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
        {[
          ...
          { label: "Total Credits Issued", value: loading ? "–" : `₹${stats.totalCredits.toLocaleString("en-IN")}`, icon: IndianRupee, bg: "bg-emerald-50", color: "text-emerald-600" },
        ].map(c => (
      ...
      <div className="mt-6 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <div className="relative w-full sm:w-64">
          ...
          <Input placeholder="Search referrer or referred..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 h-9 text-sm" />
        </div>
      </div>
      ...
      <div className="mt-4 bg-card rounded-xl border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="p-4"><Skeleton className="h-16" /><Skeleton className="h-16 mt-2" /></td></tr>
              ) : filtered.length === 0 ? (
